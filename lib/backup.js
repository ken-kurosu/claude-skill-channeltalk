...
function save(target, id, data, meta = {}) {
  ...
  try {
    ...
    execSync(`ssh -o ConnectTimeout=5 -o BatchMode=yes ${AIKA_HOST} 'mkdir -p ${AIKA_BACKUP_DIR}' 2>/dev/null`, { stdio: 'pipe' });
    execSync(`scp -o ConnectTimeout=5 -o BatchMode=yes -q '${filepath}' ${AIKA_HOST}:${AIKA_BACKUP_DIR}/`, { stdio: 'pipe' });
  } catch (e) {
...
function load(filename) {
  ...
  if (!fs.existsSync(fp)) {
    ...
    try {
      ...
      const AIKA_HOST = process.env.AIKA_HOST || 'kurosu@100.84.67.39';
      const AIKA_BACKUP_DIR = '/Users/kurosu/channeltalk-skill-backups';
      execSync(`scp -o ConnectTimeout=5 -o BatchMode=yes -q ${AIKA_HOST}:${AIKA_BACKUP_DIR}/${filename} '${fp}'`, { stdio: 'pipe' });
...
function listOnAika() {
  try {
    ...
    const AIKA_HOST = process.env.AIKA_HOST || 'kurosu@100.84.67.39';
    const AIKA_BACKUP_DIR = '/Users/kurosu/channeltalk-skill-backups';
    const out = execSync(`ssh -o ConnectTimeout=5 -o BatchMode=yes ${AIKA_HOST} 'ls -t ${AIKA_BACKUP_DIR}/ 2>/dev/null | head -50'`, { encoding: 'utf8' });
